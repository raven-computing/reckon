...
export const constants = { dec: 42, float: 3.14, hex: 0xff, oct: 0o17, bin: 0b1010, big: 9007199254740993n, str: "text", tpl: `value:${1 + 2}`, re: /foo+/gi, nil: null, bool: true, };
...
@sealed export class Sample extends Base {
    static counter = 0;
    static ["computed" + "Name"] = "ok";
    #secret = 7;
    static { this.counter = 1; }
    constructor(name = "sample") { super(name); }
    get value() { return this.#secret; }
    set value(next) { this.#secret = next; }
    #privateMethod(step = 1) { this.#secret += step; }
    *iter(times = 2) {
    ...
    method(value) {
        ...
        try {
            ...
            with ({ value }) { debugger; value = value + 1; }
        } catch (error) {
...
export default function makeSample() {
    ...
    sample.value ??= 1;
    const object = { __proto__: null, sample, regular, arrow, conciseObject, generator, asyncFn, readFile, fsPromises, maybeFn, ternary, sequence, };
    Object.defineProperty(object, "hidden", { value: true, enumerable: false });
    return object;
}

export { Base, regular, arrow, conciseObject, generator, asyncFn, tail };
