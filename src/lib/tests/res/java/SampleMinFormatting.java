...
@FunctionalInterface interface MyFunctional { int apply(int x, int y); }

@SuppressWarnings({"unchecked", "deprecation"})
public class Sample<T extends Number & Comparable<T>> extends AbstractList<T> implements Serializable {
    // Fields
    ...
    // Annotation
    @interface MyAnnotation { String value(); int number() default 42; }
    // Record
    ...
    // Constructor
    public Sample() { super(); this.publicField = 0; }
    ...
    public <E extends Exception> T exampleMethod(int a, String... args) throws E {
        ...
        if (a > 0) {
            for (int i = 0; i < a; i++) {
                ...
                } catch (Exception ex) { throw (E) ex; } finally { System.out.println("Finally"); }
            }
        } else if (a == 0) { while (a < 10) { a++; }
        } else { do { a--; } while (a > -10); }
        assert a != 42 : "a should not be 42";
        return null;
    }
    // Overridden method
    @Override public T get(int index) { return privateList.get(index); }
    @Override public int size() { return privateList.size(); }
    // Static method
    ...
    // Static nested class
    public static class Nested { public static void nestedMethod() {} }
    // Anonymous class
    Runnable anon = new Runnable() {
        @Override public void run() { System.out.println("Anonymous class"); }
    };
    // Generic method
    public <U> U genericMethod(U param) { return param; }
    // Varargs method
    ...
    public static void main(String[] args) {
        ...
        sample.anon.run();
        synchronized (sample) { Nested.nestedMethod(); }
