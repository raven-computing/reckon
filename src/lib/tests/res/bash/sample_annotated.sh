...
if [[ -n "$plain" && "$plain" =~ ^[[:alpha:]]+$ ]]; then # +1 (if statement)
  ...
  : # +1 (command)
elif test -e /dev/null; then # +1 (elif clause)
  : # +1 (command)
else # +1 (else clause)
  : # +1 (command)
fi
...
# Miscellaneous
# =================================================================================================
my_var="$(ls -lh |grep "some_file.txt" |xargs)"; # +1 (variable assignment)
my_var=42; # +1 (variable assignment)
if (( my_var != 42 )); then # +1 (if statement)
  echo "Yes"; # +1 (command)
fi
