...
label <- "Alice"
flag <- FALSE
nums <- c(1L, 2L, 3L, 4L, 5L)
mat <- matrix(1:6, nrow = 2L)
100 -> rhs_val
extra = "extra"
...
doubled_list <- lapply(nums, function(n) {
    n * 2L
})
...
filtered <- nums[nums > 2L]
nums[1] <- 10L
total <- sum(nums)
avg <- mean(nums)
doubled_v <- nums * 2L
sorted_v <- sort(nums, decreasing = TRUE)

# ============================================================================
# Pipe operator
# ============================================================================

pipe_result <- 1:10 |> sum()
pipe2 <- nums |> rev() |> cumsum()
