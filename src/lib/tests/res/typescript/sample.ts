...
export class Sample extends Base implements Configurable<{ step: number }> {
    ...
    constructor(name: string = "sample") {
        super(name, Sample.counter);
    }
    ...
    configure(opts: Partial<{ step: number }>): void {
    ...
    #privateMethod(step: number = 1): void {
        this.#secret += step;
    }
