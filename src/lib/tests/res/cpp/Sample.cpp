...
template<>
const char* clamp<const char*>(
    const char* value,
    const char* min_val,
    const char* max_val
) {
    if (std::string(value) < std::string(min_val)) {
        return min_val;
    }
    if (std::string(value) > std::string(max_val)) {
        return max_val;
    }
    return value;
}
...
std::unique_ptr<geometry::Shape> createShape(const std::string& type) {
    if (type == "circle") {
        return std::make_unique<geometry::Circle>(5.0);
    } else if (type == "rectangle") {
        return std::make_unique<geometry::Rectangle>(3.0, 4.0);
    } else {
        return nullptr;
    }
}
