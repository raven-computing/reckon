...
def func_posonly_kwonly[T](x: T, /, y: int = 0, *, z: str = "z", **kw: Any) -> T:
    ...
    dc = {i: i * i for i in range(5)}
    gc = (i for i in range(3))
    first, *mid, last = [1, 2, 3, 4, 5]
    f = lambda t: t + 1
    _ = f(y)
    try:
        if y < 0: raise ValueError("neg")
    except ValueError as e: raise RuntimeError("wrapped") from e
    else: pass
    finally: y = y  # NO-OP
    return x
...
def matcher(obj: Any) -> str:
    match obj:
        case 0 | 1: return "small-int"
        case [a, b, *rest]: return f"seq {a=} {b=} {rest=}"
        case {"k": v, **rest}: return f"map {v=} {rest=}"
        case Point(x=px, y=py) if px == py: return "diag"
        case Point() as p: return f"point {p.x},{p.y}"
        case _: return "other"
...
async def async_features() -> list[int]:
    out: list[int] = []
    async with acm("v") as v, acm(v + "2") as w: _ = (v, w)
    async for item in agen(): out.append(item)
    return out
...
def exception_groups_parse_only() -> None:
    if False:
        try: raise ExceptionGroup("eg", [ValueError("v"), TypeError("t")])
        except* ValueError as eg: _ = eg
...
def misc_parse_only() -> None:
    if False:
        ...
        _ = {k: (v := k * 2) for k in range(3) if v >= 0}
        _ = [i for i in range(10) if (j := i % 3) != 0]
