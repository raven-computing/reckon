...
void testCountStatisticsAllOperationsWithAllDefaultOptions(void) {
    ...
    TEST_ASSERT_EQUAL_INT(50, stats->totalPhysicalLines);
    TEST_ASSERT_EQUAL_INT(104, stats->totalWords);
    TEST_ASSERT_EQUAL_INT(941, stats->totalCharacters);
    TEST_ASSERT_EQUAL_INT(941, stats->totalSourceSize);
    TEST_ASSERT_EQUAL_INT(0, stats->logicalLines[RCN_TEXT_UNFORMATTED]);
    TEST_ASSERT_EQUAL_INT(16, stats->logicalLines[RCN_LANG_JAVA]);
