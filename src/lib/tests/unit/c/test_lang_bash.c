...
void testBashLogicalLineCountIsCorrect(void) {
    ...
    TEST_ASSERT_EQUAL_INT(295, result.count);
...
void testBashPhysicalLineCountIsCorrect(void) {
    ...
    freeSourceFile(file);
    TEST_ASSERT_TRUE(result.state.ok);
    TEST_ASSERT_EQUAL_INT(524, result.count);
    TEST_ASSERT_EQUAL_INT(RCN_ERR_NONE, result.state.errorCode);
    TEST_ASSERT_NULL(result.state.errorMessage);
}
...
void testBashLogicalLineCountMarksAreCorrect(void) {
    ...
    TEST_ASSERT_EQUAL_INT(23814, actual.size);
...
void testBashLogicalLineCountMarksForFilePathInput(void) {
    ...
    TEST_ASSERT_EQUAL_INT(23814, annotated.size);
...
void testBashCountAllIsCorrect(void) {
    ...
    TEST_ASSERT_EQUAL_INT(885, stats->totalLogicalLines);
    TEST_ASSERT_EQUAL_INT(1572, stats->totalPhysicalLines);
    TEST_ASSERT_EQUAL_INT(60972, stats->totalSourceSize);
    TEST_ASSERT_EQUAL_INT(885, stats->logicalLines[RCN_LANG_BASH]);
    TEST_ASSERT_EQUAL_INT(1572, stats->physicalLines[RCN_LANG_BASH]);
    TEST_ASSERT_EQUAL_INT(60972, stats->sourceSize[RCN_LANG_BASH]);
